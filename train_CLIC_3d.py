...
from utils.contrast import momentum_update, l2_normalize, ProjectionHead
from utils.sinkhorn import distributed_sinkhorn
from timm.models.layers import trunc_normal_
from einops import rearrange, repeat
from utils.loss_proto import PixelPrototypeCELoss,PPD,PPC
from timm.models.layers import DropPath, to_3tuple, trunc_normal_
from dataloaders import utils
from dataloaders.LungVessel import (LungVessel, CenterCrop, RandomCrop,
...
parser.add_argument('--root_path', type=str,
                    default='../data/LungVessel55', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='Lungvessel/Lungvessel_ICLC_rca_Training2', help='experiment_name')
...
parser.add_argument('--labeled_num', type=int, default=8,
                    help='labeled data')
parser.add_argument('--total_labeled_num', type=int, default=40,
...
def train(args, snapshot_path):
    ...
    db_train = LungVessel(base_dir=train_data_path,
                          split='train',
                          num=None,
    ...
    ppce_loss = PixelPrototypeCELoss()
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs1 = model1(input_volume_batch)
            outputs_soft1 = torch.softmax(outputs1, dim=1)

            outputs2 = model2(labeled_volume_batch)
            outputs_soft2 = torch.softmax(outputs2, dim=1)
            model2_output_ux0 = model2(unlabeled_volume_batch_0)

            model2_output_ux0_soft = torch.softmax(model2_output_ux0, dim=1)
            model2_output_ux1 = model2(unlabeled_volume_batch_1)
            model2_output_ux1_soft = torch.softmax(model2_output_ux1, dim=1)
            batch_pred_mixed_batch = model2_output_ux0 * \
                                     (1.0 - ict_mix_factors) + model2_output_ux1 * ict_mix_factors

            batch_pred_mixed = model2_output_ux0_soft * \
                               (1.0 - ict_mix_factors) + model2_output_ux1_soft * ict_mix_factors
            ...
            pseudo_outputs2 = torch.argmax(batch_pred_mixed.detach(), dim=1, keepdim=False)

            pseudo_supervision1 = ce_loss(outputs1[args.labeled_bs:], pseudo_outputs2)
            pseudo_supervision2 = ce_loss(batch_pred_mixed_batch, pseudo_outputs1)

            model1_loss = loss1 + consistency_weight * pseudo_supervision1
            model2_loss = loss2 + consistency_weight * pseudo_supervision2


            proto_ul = torch.cat([outputs1[args.labeled_bs:], batch_pred_mixed_batch], 0)
            proto_ulp = torch.cat([pseudo_outputs1, pseudo_outputs2], 0)
            loss_protol = ppce_loss(proto(outputs1[:args.labeled_bs], label_batch[:][:args.labeled_bs]),
                label_batch[:][:args.labeled_bs].long())+ppce_loss(proto(outputs2[:args.labeled_bs], label_batch[:][:args.labeled_bs]),label_batch[:][:args.labeled_bs].long())
            loss_protou = ppce_loss(proto(proto_ul, proto_ulp),proto_ulp)


            loss = model1_loss + model2_loss + loss_protol+1 * loss_protou
